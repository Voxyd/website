...
class Routing {
	...
	function route() {
		...
		if(!isset($_COOKIE['lang']))
			setcookie('lang', DEFAULT_LANGUAGE, time()+365*24*3600);
		elseif(file_exists(DIR_LANGUAGE.htmlentities($_COOKIE['lang'])))
			$_json = file_get_contents(DIR_LANGUAGE.htmlentities($_COOKIE['lang']));
		$txt = json_decode($_json);
		//
